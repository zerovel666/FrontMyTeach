...
function validateAnswer() {
    const value = localTask.value.answer.str_value;
    const min = 1;
    const max = localTask.value.question.length;

    if (value < min || value > max) {
        localTask.value.answer.str_value = null;
        notificationRef.value.showNotification(`Ответ должен быть в диапазоне от ${min} до ${max}`);
    } else {
        localTask.value.answer.str_value = value;
    }
};
...
async function save() {
    ...
    if (localTask.value.answerEditor.code === 'MATCHING') {
        ...
        params = {
            json_value: JSON.stringify(matchingBody.value)
        };
    }
    else if (localTask.value.answer.str_value) {
        params = {
